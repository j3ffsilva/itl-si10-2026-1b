# Freddy Mester Harari - Exercício 8.2

## `entregas/07-freddy-mester-harari/1mte_RJsDljkcj2aItmbyeSsBJ3J0-JNL.ipynb` - cell 095 `code`
...
## `entregas/07-freddy-mester-harari/1mte_RJsDljkcj2aItmbyeSsBJ3J0-JNL.ipynb` - cell 096 `markdown`

```text
### Respostas — Exercício 8.2

**1. O que acontece com o teste de aderência aplicado a `tempo_leitura_segundos`?**

A base modificada piora neste eixo. A distribuição uniforme tem formato
"retangular", sem um pico inicial e sem cauda direita longa. Um teste de
aderência contra a log-normal teórica do E02 detectaria desvio significativo
(p << 0,05), enquanto a base original passaria. O skewness cai de ~2,6
(log-normal, assimétrica) para ~1,0 (uniforme, mais simétrica), o que indica
que a forma da distribuição mudou substancialmente.

**2. O que acontece com a coerência entre `tempo_leitura_segundos` e `clicou_comunicado`?**

A direção é preservada, quem clicou ainda tem mediana de leitura maior.
Porém os valores absolutos ficam irrealistas (mediana de ~114s para quem
não clicou é implausível numa sessão de intranet). O sinal comportamental
sobrevive, mas enfraquecido e com menos credibilidade empírica.

**3. A nova base é "melhor" ou "pior"? Em qual critério?**

| Eixo              | Original (log-normal) | Modificada (uniforme)     |
|-------------------|-----------------------|---------------------------|
| Fidelidade        |  compatível com E02  |  desvio detectável       |
| Coerência interna |  sinal forte e realista |  direção ok, valores irrealistas |
| Realismo          | cauda direita plausível |  histograma plano      |

A base uniforme é **pior nos três eixos**. Não há nenhum critério em que
ela seja superior. Aprende-se com isso que, trocar a distribuição sem recalibrar os parâmetros, ao propósito destrói fidelidade sem ganho de coerência. Toda mudança no
gerador precisa ser reavaliada no scorecard completo.
```
